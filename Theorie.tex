...
to a sum, this is called quadrature: \cite{trp} \cite{ctiaat}

\begin{equation}
\text{TRP} = \frac{\pi}{2NM}\sum^{N-1}_{i=1}\sum^{M-1}_{j=0}\left(\text{EIRP}_\theta\left(\theta_i,\phi_j\right)+\text{EIRP}_\phi\left(\theta_i,\phi_j\right)\right)\cdot\sin\left(\theta_i\right)
\label{eq:trpsumsin}
\end{equation}

$N$ is the number of angular intervals in $\theta$ (elevation), $M$ is the number of angular intervals in $\phi$ (azimuth) and the \ac{EIRP} is split in its horizontal ($\text{EIRP}_\phi$) and its vertical ($\text{EIRP}_\theta$) part. This quadrature is called sine theta. Because the sine of $0$ and $\pi$ is $0$ the summation in elevation is from $i=1$ to $N-1$.

\subsection{Clenshaw-Curtis}

Other weights instead of $\sin\left(\theta_i\right)$ can be introduced and the \ac{TRP} formula \ref{eq:trpsumsin} develops to:

\begin{equation}
\text{TRP} = \frac{\pi}{2NM}\sum^{N-1}_{i=1}\sum^{M-1}_{j=0}\left(\text{EIRP}_\theta\left(\theta_i,\phi_j\right)+\text{EIRP}_\phi\left(\theta_i,\phi_j\right)\right)\cdot\text{W}\left(\theta_i\right)
\label{eq:trpsumcc}
\end{equation}
