...
\section{Radiation Test Sites}
The dimensions of a conventional test range can be reduced by making measurements in the \acf{NF} of the Antenna under Test (AUT), and then using analytical methods to transform the measured Near-Field data to compute the Far-Field radiation characteristics. Such techniques are usually used to measure patterns and gain. \cite{balanis} \\

There are three types of \acf{NF} set-ups; planar, cylindrical and spherical. A probe is used to scan the field over this preselected geometrical surface and thus the \acf{NF} data is measured. The \acf{NF} to \acf{FF} transformation is performed using analytical methods. The complexity of the analytical transformations increases from the planar to the cylindrical, and then to the spherical surfaces. \\
