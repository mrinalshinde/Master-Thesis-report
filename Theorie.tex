...
\begin{equation}
\color{blue}\left(2\Delta\Phi_{\text{ref}}\right)^{-1}\color{black},\ \color{cyan}\left(2\Delta\Phi'\right)^{-1}\color{black}\ \text{and}\ \color{green}\text{first mode} > \SI{-40}{\decibel} \color{black}
\end{equation}
...
The issue is now to find the sampling frequency that the sampling error is independent from the aperture of the antenna. As depicted in figure \ref{fig:evolvpattern3} the sampling frequency introduced by \cite{hansen} is to strict for small apertures. On the other hand the reference angle introduced by \cite{2018arXiv180310993F} is to inaccurate for small apertures. Because of that a new factor was taken:

\begin{equation}
\text{CrefA} = \frac{\Delta\Phi_{\text{ref}}\cdot\left(\text{first mode} > \SI{-40}{\decibel}\right)}{2}
\end{equation} 

This factor is plotted in \ref{fig:crefa}. It is dependent of the aperture.
...
So the error due to sampling is independent from the antennas aperture.\\
For further reduction of points a second factor is introduced, the Sparsity Factor SF. At $\text{SF}=2$ every second skipped. With all that the angle increments for spherical sampling is:
...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{Matlab/GridCoDePolar.eps}
\caption{Constant Density Grid with 264 Measurementpoints}
\label{fig:cdg}
\end{figure}

The \ac{CDG} depicted in figure \ref{fig:cdg} \todo{hier weiterschreiben}
