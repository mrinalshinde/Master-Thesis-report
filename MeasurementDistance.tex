...
In the simulation it is assumed that the characteristics and behaviour of the \ac{DUT} is independent from the probe, thus the vector-EM-field in the volume is independent from the used sampling probe. This is a valid assumption for \ac{FF} or for probes with little scattering back to the \ac{DUT}. This is visual for the Vivaldi probe comparing simulation results from figure \ref{fig:simres} (a) and the measurement results in figure \ref{fig:meastrpdistviv} (a). Bigger impact is recognized comparing the graph of the \ac{SGH} probe simulation to the measurement. This can be caused by the greater amount of metal put in the \ac{NF}-volume of the \ac{DUT}. In addition the shape of the graphs of the \ac{OEWG}2 and the patch are having less \ac{TMP}-error in the \ac{FF} of the \ac{DUT}. Caused by the change of the \ac{EM} vectorfield from the \ac{DUT} the measurement result is sharply rising till it is similar with the $\SI{16}{\decibel}$ more directive \ac{SGH}. This phenomena is also visible plotting the \ac{EIRP}-pattern of the \ac{SGH} \ac{DUT} in different measurement radii. Comparing the measured pattern from the Vivaldi probe (figure \ref{fig:sghpatternvivaldi}) to the \ac{OEWG}2 (figure \ref{fig:sghpatternoewg2}) and the patch (figure \ref{fig:sghpatternpatch}) additionally to the evolving pattern over distance (compare figure \ref{fig:devantennap}) some fluctuations in the surrounding of the main lobe are recognisable. The center of rotation is chosen to be the center of the aperture $\SI{3}{\centi\meter}$ away from the phase center of the horn, leading to fine range length differences and thereby to different standing wave modes explaining the fluctuation.
